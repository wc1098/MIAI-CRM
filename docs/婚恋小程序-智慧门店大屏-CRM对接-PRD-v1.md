...
- 销售私海：已分配到销售名下。
- 销售默认不允许自由领取门店公海线索，需由店长分配；后续可做为品牌/门店配置项。

**状态维度（已确认）**
- 线索采用“双维度”管理：归属池 + 销售阶段。
- 归属池：品牌公海 / 门店公海 / 销售私海。
- 销售阶段：新线索 / 跟进中 / 已邀约 / 已到店 / 已面谈 / 已转客户 / 已成交 / 无效。
...
- 回收、改派、跨店转交需填写原因。
- 自动回收规则（已确认）：先做提醒和手动回收；自动回收作为配置能力保留，但默认关闭。
...
**跟进记录**
- 跟进类型包括：电话、微信、短信、到店邀约、活动邀约、资料补全、产品介绍、异议处理、其他。
- 跟进记录字段：跟进方式、跟进时间、跟进内容、客户意向、下次跟进时间、关联线索/客户、提交人。
- 销售只能给自己名下线索/客户新增跟进。
- 店长可查看本门店全部跟进。
- 跟进记录不允许物理删除，可作废并留痕。
...
**无效与重新激活（已确认）**
- 无效线索允许重新激活。
- 重新激活需填写原因，并写入 SourceEvent / 生命周期流转记录。
...
**销售端 - 线索**
- 列表筛选：状态、来源、意向等级、分配时间、是否超时未跟进、最近跟进时间。
- 详情页：基础资料、来源事件、跟进记录、邀约记录、到店/面谈、转客户、无效/激活。
...
- 线索列表、详情、状态推进、跟进记录
- 分配/转交（严格门店隔离，只能转交）
- 线索去重：手机号按全品牌唯一主体去重；重复触达不重复创建 Person，追加来源事件与业务记录。
- 线索池：支持品牌公海、门店公海、销售私海三层；销售默认不可自由领取门店公海线索，由店长分配，后续可配置。
- 状态维度：采用“归属池 + 销售阶段”双维度管理。
- 提醒：新分配线索 24 小时未首次跟进提醒；跟进中线索 7 天无跟进提醒。
- 回收：首版以提醒和手动回收为主，自动回收作为配置能力保留但默认关闭。
- 无效线索：允许重新激活，需填写原因并记录事件。
- 线索转客户：创建客户档案并建立合同/服务期（可先最小）
...
| 线索管理 | 列表、详情、分配、领取配置、回收、转交、作废、激活 | 总部池/门店公海/销售私海分层过滤 | 列表脱敏；完整手机号按归属查看 | 查看完整手机号、分配、回收、转交、激活、作废 |
...
| `resource_flow_log` | 记录线索/客户/VIP 在池子、人员、门店之间流转 | 支撑公海、私海、转交、回收、重新激活 |
...
| `pool_type` | enum | 是 | hq_pool/store_pool/sales_private/invalid/converted |
| `owner_sales_id` | bigint | 否 | 当前销售 |
| `intention_level` | enum | 否 | 意向等级 |
| `lead_stage` | enum | 是 | new/following/appointed/visited/consulted/signing_ready/invalid/converted |
| `latest_source_event_id` | bigint | 否 | 最近来源事件 |
| `latest_follow_at` | datetime | 否 | 最近跟进时间 |
| `next_follow_at` | datetime | 否 | 下次跟进时间 |
| `no_first_follow_reminded` | bool | 是 | 24 小时未首跟提醒标记 |
| `no_follow_7d_reminded` | bool | 是 | 7 天未跟进提醒标记 |
| `invalid_reason` | varchar/text | 否 | 无效原因 |
| `reactivate_reason` | varchar/text | 否 | 重新激活原因 |
| `status` | enum | 是 | active/invalid/converted |
| `created_at` | datetime | 是 | 创建时间 |
| `updated_at` | datetime | 是 | 更新时间 |
...
- Lead 转 VIP 时必须自动补齐 `customer_profile`。
...
| D007 | 自动回收先配置但默认关闭 | 先人工提醒和手动回收 | 线索运营 |
