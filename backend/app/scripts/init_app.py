...
async def lifespan(app: FastAPI) -> AsyncGenerator[Any, Any]:
    ...
    try:
        ...
        from app.plugin.module_crm.lead.service import LeadService
...
def register_routers(app: FastAPI) -> None:
    ...
    from app.plugin.module_screen.ws import ScreenWsRouter
    ...
    app.include_router(
        router=ScreenWsRouter, dependencies=[Depends(WebSocketRateLimiter(times=5, seconds=5))]
    )
    # 先将动态路由注册到应用，使用速率限制器
