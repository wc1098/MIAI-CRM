...
function shouldProcess(url?: string) {
...
function appendProcess(url: string, process: string) {
  if (!shouldProcess(url)) return url;
  const separator = url.includes("?") ? "&" : "?";
  return `${url}${separator}x-oss-process=${encodeURIComponent(process)}`;
}

export function ossImage(url?: string, options: OssImageOptions = {}) {
  if (!url) return "";
  ...
  return appendProcess(url, `image/resize,${resize}/quality,q_${quality}/format,${format}`);
}

export function ossPreview(url?: string, options: OssImageOptions = {}) {
  if (!url) return "";
...
export function ossImageList(urls?: string[], options: OssImageOptions = {}) {
